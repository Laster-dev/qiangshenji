    ...
    public partial class Form1 : MyForm
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (hWnd==Program.Qsj) {
                ...
                for (int j = 0; j < 3; j++) {
                    if (WinApi.GetPixelColor(Program.Qsj, selectedPoint.X, selectedPoint.Y) != GameColor)
                    {
                        i++;
                    }
                }
        ...
        private async void 自动背刺ToolStripMenuItem_ClickAsync(object sender, EventArgs e)
        {
            ...
            if (自动背刺ToolStripMenuItem.Checked)
            {
                ...
                points = await WinApi.ScanColorAsync(Program.Qsj, Color.FromArgb(12, 230, 0), scanArea);
                Program.ShowNotification($"扫描完毕！获取到{points.Count}个关键点", 3000);
                timer1.Enabled = true;
            }
